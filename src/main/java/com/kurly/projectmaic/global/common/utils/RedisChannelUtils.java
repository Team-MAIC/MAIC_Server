...
public class RedisChannelUtils {
	...
	public static ChannelTopic getDasTodoTopic(final long centerId, final CenterProductArea area) {
		String channelName = String.format(PICK_TODOS_TOPIC_FORMAT, centerId, area);

		return new ChannelTopic(channelName);
	}

	public static String getDasTodoTopicName(final long centerId, final int passage) {
		return String.format(PICK_TODOS_TOPIC_FORMAT, centerId, passage);
	}
}
